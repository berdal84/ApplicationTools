...
MemoryPool::MemoryPool(uint _objectSize, uint _objectAlignment, uint _blockSize)
	...
	, m_blockCount(0)
{
	PLR_ASSERT(m_objectSize >= sizeof(void*)); // objects must be at least the size of a ptr
}

MemoryPool::~MemoryPool()
{
	PLR_ASSERT(m_usedCount == 0); // not all objects were freed
...
void MemoryPool::allocBlock()
{
	...
	m_blocks[m_blockCount] = plr::malloc_aligned(m_objectSize * m_blockSize, m_objectAlignment);
