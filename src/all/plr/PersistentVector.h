...
////////////////////////////////////////////////////////////////////////////////
#pragma once
#ifndef plr_PersistentVector_h
#define plr_PersistentVector_h
...
inline void PersistentVector<tType>::allocBlock()
{
	...
	m_blocks[m_blockCount] = (tType*)plr::malloc_aligned(sizeof(tType) * m_blockSize, PLR_ALIGNOF(tType));
	++m_blockCount;
}
...
public:
	...
	{
		PLR_ASSERT(*this != m_parent->end()); // can't increment the end iterator
		++m_offset;
	...
	iterator_base& operator--()
	{
		PLR_ASSERT(*this != m_parent->begin()); // can't decrement the begin iterator
		if (m_offset == 0) {
			--m_block;
	...
	{
		PLR_ASSERT(m_block < m_parent->m_blockCount && m_offset < m_parent->m_blockSize);
